...
def check_api_keys_status(api_keys: list, model: str = None) -> dict:
    """Check validity of one or more Mistral API keys."""
    results = {}
    client_model = model or "mistral-small-latest"
    for key in api_keys:
        try:
...
def get_mistral_metadata(
    ...
    """Send an image to Mistral and return parsed metadata dict."""
    if check_stop_event(stop_event, "Mistral: stop requested before request"):
    ...
    model = selected_model_input or "mistral-small-latest"
    ...
    try:
        ...
        if check_stop_event(stop_event, "Mistral: stop requested after response"):
            return {"error": "stopped"}
        ...
        log_message("Metadata successfully extracted from Mistral response", "success")
        return metadata

    except Exception as e:
        log_message(f"Mistral request failed: {e}", "error")
        return {"error": str(e)}
